...
class Expansion_OneShot_0ed(Method):
    ...
    def expand(
        ...
        *,
        ...
        High-level overview:
          ...
          6. Materialize new child nodes at their parent positions (no noise).
        ...
        for li, sc in zip(leaf_idx.tolist(), spawn_counts_final.tolist()):
            if sc == 0:
                continue
            parent_pos = pos[li]
            child_pos = parent_pos.unsqueeze(0).repeat(sc, 1)
    ...
    # ---------------------------------------------------------
    # 1) Build masked positions: replace leaf coords by parent positions
    # ---------------------------------------------------------
    @th.no_grad()
    def _make_masked_positions(
        ...
        *,
        ...
        """
        Returns pos_in: [N,3] where leaves are replaced by parent_pos (no noise).
