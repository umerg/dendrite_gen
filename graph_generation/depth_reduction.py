...
class _DepthCherryState:
    ...
    # Depth bookkeeping for deepest-level cherry reductions
    depth: Dict[Node, int]
    max_depth: int

class DepthCherryReducer:
    """
    Cherry contraction variant that only removes cherries whose leaf children sit
    at the deepest level of the current rooted tree (root may be k-ary).
    - 'Cherry' = parent whose ALL children are leaves (>=1 child).
    - Per level, prune:
        * deterministic: all deepest-level cherries
        * stochastic: each deepest-level cherry with prob p (ensuring at least one)
    - Returns a NEW DepthCherryReducer with updated state, new adj, and
    ...
    # ------------------------------------------------------------------
    def get_reduced_graph(self) -> "DepthCherryReducer":
        """
        Execute one cherry-prune level and return the next DepthCherryReducer:
        ...
        # Restrict to cherries whose child leaves sit at the current deepest depth.
        deepest_depth = S.max_depth
        deepest_cherries: List[Node] = []
        for u in cherries_all:
            children_u = S.children[u]
            if not children_u:
                continue
            child_depths = [S.depth[ch] for ch in children_u]
            if child_depths and max(child_depths) == deepest_depth:
                deepest_cherries.append(u)
        cherries_all = deepest_cherries

        self.new_leaves_from_next = np.empty(0, dtype=np.int64)
        if not cherries_all or self.n <= 1:
            # No further contraction; return a "no-op" next level
            cr = DepthCherryReducer(
                adj=self.adj,
                root=S.root,
                mode=self.mode,
        ...
        new_max_depth = max(new_depth.values()) if new_depth else 0
...
class DepthReductionFactory:
    """
    Minimal factory that instantiates a DepthCherryReducer for a given adjacency.
    ...
    def __call__(self, adj: sp.spmatrix, rng=None) -> DepthCherryReducer:
        root = self._resolve_root(adj)
        return DepthCherryReducer(
            adj=adj,
            root=root,
            mode=self.mode,
            cherry_p=self.cherry_p,
