...
for NAME in MAX_TREES_PER_CLASS PAIRS_PER_SHARD SO2_GRID_SIZE CPUS_PER_JOB MAX_CONCURRENT_JOBS; do
  VALUE="${!NAME}"
...
IFS= read -r SHARD_COUNT < "${TASK_COUNT_PATH}"
if [[ ! "${SHARD_COUNT}" =~ ^[1-9][0-9]*$ ]]; then
  echo "Invalid task count in ${TASK_COUNT_PATH}: ${SHARD_COUNT}" >&2
  exit 2
fi
ARRAY_TASK_COUNT=$(((SHARD_COUNT + CPUS_PER_JOB - 1) / CPUS_PER_JOB))
ARRAY_END=$((ARRAY_TASK_COUNT - 1))

COMMON_EXPORT="ALL,PERMANENT_STORAGE_DIR=${PERMANENT_STORAGE_DIR},PROJECT_ROOT=${PROJECT_ROOT},REPOSITORY_DIR=${REPOSITORY_DIR},CONDA_ROOT=${CONDA_ROOT},CONDA_ENVIRONMENT=${CONDA_ENVIRONMENT},OUTPUT_DIR=${OUTPUT_DIR},SHARD_COUNT=${SHARD_COUNT}"
...
echo "Array job: ${ARRAY_SUBMISSION} (${ARRAY_TASK_COUNT} jobs for ${SHARD_COUNT} shards)"
echo "Maximum allocation: ${MAX_CONCURRENT_JOBS} jobs x ${CPUS_PER_JOB} CPUs"
