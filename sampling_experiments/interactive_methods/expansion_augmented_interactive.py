"""Interactive wrapper for the augmented expansion method."""
...
from graph_generation.method.expansion_oneshot_augmented_edges import (
    Expansion_OneShot_Augmented,
)


@dataclass
class AugmentedGraphStepTrace:
    """Per-step trace schema for the augmented expansion variant."""
...
class InteractiveExpansionOneShotAugmented(Expansion_OneShot_Augmented):
    """Augmented method that will emit rich traces for notebooks."""
    ...
    def sample_graphs_with_trace(
        ...
        *,
    ...
    ) -> tuple[list[nx.Graph], list[list[AugmentedGraphStepTrace]]]:
        if target_size.dim() != 1:
            raise ValueError("target_size must be 1D tensor of capacities.")
        ...
        traces: list[list[AugmentedGraphStepTrace]] = [[] for _ in range(num_graphs)]
    ...
    def _capture_step_traces(
        ...
        *,
    ...
    ) -> list[AugmentedGraphStepTrace]:
        num_graphs = int(target_size.numel())
        traces: list[AugmentedGraphStepTrace] = []
        ...
        for g in range(num_graphs):
            ...
            rel_pred_tensor = th.tensor(info["rel_pred"], dtype=pos_cpu.dtype) if info and info.get("rel_pred") else None
            noise_tensor = th.tensor(info["noise"], dtype=pos_cpu.dtype) if info and info.get("noise") else None
            ...
            if rel_pred_tensor is not None:
                extras = extras or {}
                extras["rel_pred"] = rel_pred_tensor
            if noise_tensor is not None:
                extras = extras or {}
                extras["noise"] = noise_tensor

            trace = AugmentedGraphStepTrace(
                step_idx=step_idx,
                graph_id=g,
    ...
    def expand(
        ...
        *,
        ...
        """Augmented expand override with optional debug payload."""
        ...
        for li, sc in zip(leaf_idx.tolist(), spawn_counts_final.tolist()):
            ...
            noise = self._sample_noise(
                (sc, parent_pos.shape[0]), device, sigma=self.leaf_noise_sigma, clip=self.leaf_noise_clip
            )
        ...
        if return_debug_payload:
            noise_samples = th.cat(noise_records, dim=0) if noise_records else None
